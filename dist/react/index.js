...
export { ConnectionStatus, PaperType, PaperTypeNames } from 'detonger-web-bt';
 ...
 */
function PrinterProvider({ children, config, autoConnect = false, autoDisconnectOnUnmount = true, onConnected, onDisconnected, onError: onErrorProp, }) {
    ...
    const refreshStatus = useCallback(async (options) => {
    ...
    const printImage = useCallback(async (imageUrl, options) => {
        return printer.printImage(imageUrl, options);
    }, [printer]);
    const printCanvas = useCallback(async (canvas, options) => {
        return printer.printCanvas(canvas, options);
    }, [printer]);
    const printImageData = useCallback(async (imageData, options) => {
        return printer.printImageData(imageData, options);
    }, [printer]);
    const feedLines = useCallback(async (n) => {
    ...
    return (jsx(PrinterContext.Provider, { value: contextValue, children: children }));
}
 ...
 */
function usePrinterStatus(options) {
    ...
    const refresh = useCallback(async (full) => {
